...
# A9VG汉化版：一键生成用户指定的 8 个发布版本（与 CI compile.yml 完全一致）
#   带内置背景图（BG=1，CUSTOM_COLORS 黑底蓝选）：
#     BOOT-EXFAT.ELF（无外设标志，基础版）
#     BOOT-EXFAT-MX4SIO.ELF / BOOT-EXFAT-MMCE.ELF
#     BOOT-EXFAT-MMCE-XFROM.ELF / BOOT-EXFAT-MX4SIO-XFROM.ELF
#   无内置背景图（BG=0，自动启用灰白默认色 GREY_COLORS）：
#     BOOT-EXFAT-NOBG.ELF（无外设标志，基础版）
...
release:
	$(MAKE) clean && $(MAKE) all EXFAT=1 BG=1
	...
	$(MAKE) clean && $(MAKE) all EXFAT=1 BG=0
